#!/usr/bin/env python3
"""Unified Gacha benchmark for all Chonkie chunkers.

Tests various chunkers on the Gacha dataset using:
...
Usage:
    python gacha.py --chunkers fast token
    python gacha.py --chunkers all
    python gacha.py --chunkers fast --sizes 1024 2048 4096
    python gacha.py --list
"""
...
# Map embedding models to their corresponding tokenizers
# Format: model_name -> tokenizer_identifier
TOKENIZER_MAP = {
...
def get_chunker_configs() -> Dict[str, ChunkerConfig]:
    ...
    return {
        ...
        "sentence": ChunkerConfig(
            ...
            unit="T",
            needs_tokenizer=True,
            init_fn=lambda size, tokenizer, **_: _init_sentence_chunker(
                size, tokenizer
            ),
        ),
        "recursive": ChunkerConfig(
            ...
            unit="T",
            needs_tokenizer=True,
            init_fn=lambda size, tokenizer, **_: _init_recursive_chunker(
                size, tokenizer
            ),
        ),
        ...
        "neural": ChunkerConfig(
            ...
            sizes=[],  # Neural chunker doesn't use fixed sizes
            unit="",
            needs_tokenizer=False,
            needs_special_setup=True,
        ...
        "slumber": ChunkerConfig(
            ...
            needs_tokenizer=True,
            needs_special_setup=True,
            init_fn=None,  # Requires LLM setup
            notes="Requires LLM/genie configuration",
        ),
        "table": ChunkerConfig(
            name="TableChunker",
            description="Specialized chunker for tabular data",
            sizes=[3, 5, 10],  # Row counts
            unit="R",  # Rows
            needs_tokenizer=False,
            needs_special_setup=True,
            init_fn=lambda size, **_: _init_table_chunker(size),
            notes="For markdown/CSV tables; size = max rows",
        ),
    }
...
def _init_fast_chunker(size: int):
    ...
    return FastChunker(chunk_size=size)
...
def _init_table_chunker(size: int):
    from chonkie import TableChunker

    return TableChunker(chunk_size=size)
...
def load_or_download_corpus() -> tuple[List[Dict], List[Dict]]:
    ...
    corpus_path = DATA_DIR / "gacha_corpus.jsonl"
    questions_path = DATA_DIR / "gacha_questions.jsonl"

    if corpus_path.exists() and questions_path.exists():
        log("  Loading cached corpus and questions...")
        corpus = load_jsonl(corpus_path)
        questions = load_jsonl(questions_path)
        log(f"    Loaded {len(corpus)} books and {len(questions)} questions from cache")
    else:
        log("  Downloading from HuggingFace...")
        from datasets import load_dataset

        corpus_ds = load_dataset("chonkie-ai/gacha", "corpus", split="train")
        questions_ds = load_dataset("chonkie-ai/gacha", "questions", split="train")

        corpus = [{"title": c["title"], "text": c["text"]} for c in corpus_ds]
        questions = [
            {
                "title": q["title"],
                "question": q["question"],
        ...
        log(f"    Downloaded {len(corpus)} books and {len(questions)} questions")
...
def chunk_corpus(
    ...
    """Chunk all books and return list of chunk dicts."""
    chunks_path = DATA_DIR / f"chunks_{chunker_key}_{size}{unit}.jsonl"
    ...
    chunks = []
    skipped = 0
    for book in tqdm(corpus, desc="Chunking", leave=False):
        book_chunks = chunker.chunk(book["text"])
...
def embed_chunks(
    ...
    model_short = get_model_short_name(model)
    embeddings_path = DATA_DIR / f"chunk_embeddings_{chunker_key}_{size}{unit}_{model_short}.npy"
    ...
    # Calculate batch size based on approximate token count
    if unit == "T":
        batch_size = calculate_batch_size(size)
    elif unit == "B":
        # Approximate: 4 bytes per token
        batch_size = calculate_batch_size(size // 4)
    else:
        batch_size = 64
...
def benchmark_configuration(
    ...
    collection = chroma_client.create_collection(
        name=f"gacha_{chunker_key}_{size}{unit}",
    ...
    for i in tqdm(
        ...
        collection.add(
            ...
            metadatas=[
                {"book_title": c["book_title"], "token_count": c.get("token_count", 0)}
...
def print_chunker_list():
    ...
    print("  fast, token, sentence, recursive")
    print("\nAdvanced chunkers (require additional setup):")
    print("  semantic, late, code, neural, slumber, table")


def main():
    """Main benchmark function."""
    parser = argparse.ArgumentParser(
        description="Gacha benchmark for Chonkie chunkers",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python gacha.py --chunkers fast token          # Benchmark FastChunker and TokenChunker
  python gacha.py --chunkers all                 # Benchmark all standard chunkers
  python gacha.py --chunkers fast --sizes 2048 4096  # Custom sizes for FastChunker
  python gacha.py --chunkers fast --model text-embedding-3-large  # Use OpenAI embeddings
  python gacha.py --list                         # List all available chunkers
        """,
    )
    parser.add_argument(
        "--chunkers",
        nargs="+",
        default=["fast"],
        help="Chunkers to benchmark (fast, token, sentence, recursive, semantic, late, code, or 'all')",
    )
    parser.add_argument(
        "--sizes",
        nargs="+",
    ...
    if "all" in args.chunkers:
        ...
        chunker_keys = ["fast", "token", "sentence", "recursive"]
    else:
    ...
    log("=" * 60)
    log("Gacha Benchmark - Chonkie Chunkers")
    log("=" * 60)
    ...
    for chunker_key in chunker_keys:
        ...
        for size in sizes:
            ...
            # Chunk corpus
            chunks = chunk_corpus(corpus, chunker, chunker_key, size, config.unit)

            # Embed chunks
            chunk_embeddings = embed_chunks(
                catsu_client, chunks, chunker_key, size, config.unit, model=args.model
            )
