...
This script samples ~100 questions from each dataset, finds the relevant documents
that contain those questions' passages, and uploads the nano datasets to HuggingFace.
...
def generate_nano_dataset(
    ...
    """Generate a nano version of a dataset.
    ...
    # Build corpus text lookup for faster searching
    corpus_texts = [get_text_from_doc(doc, config["corpus_text_field"]) for doc in corpus_data]

    # Filter to only questions with valid passages (passage exists in corpus)
    ...
    for q_idx, question in enumerate(questions_data):
        ...
        if passage and any(passage in doc_text for doc_text in corpus_texts):
            valid_question_indices.append(q_idx)

    print(f"Valid questions (passage exists in corpus): {len(valid_question_indices)}/{len(questions_data)}")

    if len(valid_question_indices) < num_questions:
        print(f"WARNING: Only {len(valid_question_indices)} valid questions available, using all of them")

    # Sample from valid questions only
    rng = random.Random(seed)
    n = min(num_questions, len(valid_question_indices))
    sampled_indices = rng.sample(valid_question_indices, n)

    sampled_questions = [questions_data[i] for i in sampled_indices]
    sampled_passages = [
    ...
    # Find documents containing the sampled passages
    # Only keep ONE document per passage (the first one found) to avoid duplicates
    # from passages that appear in multiple documents
    passage_to_doc_idx = {}  # passage_idx -> doc_index

    for doc_idx, doc in enumerate(corpus_data):
        doc_text = get_text_from_doc(doc, config["corpus_text_field"])
        for pass_idx, passage in enumerate(sampled_passages):
            # Only assign if this passage doesn't already have a document
            if passage and pass_idx not in passage_to_doc_idx and passage in doc_text:
                passage_to_doc_idx[pass_idx] = doc_idx

    # Collect unique documents
    unique_doc_indices = set(passage_to_doc_idx.values())
    needed_docs = [dict(corpus_data[i]) for i in sorted(unique_doc_indices)]

    print(f"Found {len(needed_docs)} relevant documents")

    # Verify all passages are covered
    covered = 0
    ...
    return needed_docs, sampled_questions
